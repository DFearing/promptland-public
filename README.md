...
![wordmark](design/assets/logo/promptland-wordmark.svg)
...
First run: the app opens to character creation. Pick a world, species, gender, class, and a name. No accounts, no signup — everything lives in your browser's IndexedDB.
...
- **Fully wired:** character creation, full stat block, combat (with level scaling, ambush, mob self-heal, spells, conditions, DOTs), inventory (rarity + level + weight + requirements), drives + goal-seeking, map UI, fog of war, portals, three worlds, LLM integration with caching + bespoke mob/item generation.
- **Partial:** shopping (auto-buy works, no manual purchase UI); death penalties (hardcoded, design doc promised configurable); sacrifice (dev-panel only, not wired into main loop).
- **Deferred:** 2D sprite rendering (placeholder only), Cloudflare Workers backend (Track A), cross-device sync, corpses as shared-world entities.
